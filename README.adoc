...
image:https://img.shields.io/maven-central/v/org.jboss/jboss-parent[Maven Central]
...
|nexus.skipPublishing|false|Set to `true` to skip publishing.
|===
...
This move is accomplished by the nxrm3:staging-move goal of the nxrm3-maven-plugin:
...
This move is accomplished by the nxrm3:staging-delete goal of the nxrm3-maven-plugin:
...
[id='central-release-profile']
== The Central Release Profile

In addition to the normal <<the-jboss-release-profile,JBoss release>> profile, a second release profile is also supported for deploying directly to Maven Central called `central-release`.

This profile supports direct deployment to Maven Central of all artifacts.
At present, staged releases are not supported.

Activating the profile is done by executing a deployment operation like this:

.Example of deploying a project to Maven Central
[source,bash]
----
...
.Supported properties for central deployment
[cols="1m,1m,2",options="header"]
|===
|Property|Default|Description
|central.serverId|central|The server ID used for deployment.
|central.sonatype.url|https://central.sonatype.com|The URL of the central deployment target.
|central.autoPublish|true|Set to `true` to automatically publish upon successful deployment.
|central.waitUntil|validated|How long to wait after deployment before allowing the build to continue.
|central.skipPublishing|false|Set to `true` to skip publishing.
|central.deploymentName|`${project.artifactId}-${project.version}`|The name of the deployment.
|===
...
* `java25.home`: this property must be set to the location of a Java 25 JDK installation
* `java26.home`: this property must be set to the location of a Java 26 JDK installation
* `java27.home`: this property must be set to the location of a Java 27 JDK installation
...
|build-test-java26|Run tests for Java 26 when `java26.home` is set and JDK 27 or later is used.|<<mr-jar-testing>>
|build-test-java27|Run tests for Java 27 when `java27.home` is set and JDK 28 or later is used.|<<mr-jar-testing>>
|===

[id='releasing-this-project']
== Releasing this project

This project does not use the Maven Release Plugin.
Instead, releases are performed manually using the steps below.

In the examples that follow, the release version is `53` and the next development version is `54`.

=== Prerequisites

* Ensure your `settings.xml` is configured with valid credentials for the `jboss` server ID (see <<the-jboss-release-profile>>).
* Ensure your GPG key is configured for artifact signing.

=== Step 1: Update your local checkout

Fetch the latest changes and fast-forward your `main` branch:

[source,bash]
----
git fetch upstream
git merge --ff-only upstream/main
git log --oneline -5   # sanity-check the history
----

=== Step 2: Detach from `main`

Detach HEAD so the release commit is not made directly on `main`:
...
=== Step 3: Set the release version

[source,bash]
----
mvn versions:set -DnewVersion=53
----
...
[source,bash]
----
git commit -am "Prep 53"
git tag -s jboss-parent-53 -m "jboss-parent-53"
----

=== Step 5: Build and deploy

Run a clean build, then deploy to the `jboss-common` repository:

[source,bash]
----
mvn clean install
mvn deploy -Pjboss-release -Dnexus.repository.release=jboss-common
----

=== Step 6: Advance the `main` branch to the next snapshot

[source,bash]
----
git checkout main
mvn versions:set -DnewVersion=54-SNAPSHOT
git commit -am "Next is 54"
----

=== Step 7: Push

Do a dry run first to verify what will be pushed:

[source,bash]
----
git push --dry-run upstream main jboss-parent-53
----

If everything looks correct, push for real:

[source,bash]
----
git push upstream main jboss-parent-53
----
